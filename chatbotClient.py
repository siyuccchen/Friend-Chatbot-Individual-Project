...
class ChatBox:
    def __init__(self):
        ...
        self.text_area.configure(cursor = "arrow" , state = DISABLED,wrap=WORD)
        ...
        self.scroll.place(relheight = 1, relx = 0.985)
        self.scroll.configure(command = self.text_area.yview,width=14)
    ...
    def sendMassage(self, msg, sender):
        ...
        rep=soc.recv(8192).decode("utf-8")
        if msg=="bye":
            soc.close()
            sys.exit()
        msg2 = rep+"\n\n"
